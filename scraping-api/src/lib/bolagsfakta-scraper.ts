...
import { bolagsfaktaPipelineListLog } from './bolagsfakta-pipeline-list-file-logger.js'
import {
  normalizeOrgNumber,
...
/**
 * Bolagsfakta använder oftast ?sida=N; vissa sidor kan använda ?page=N.
 * Vi väljer det som faktiskt förekommer i list-HTML, annars default sida.
 */
...
/** Hitta faktisk href för sida N (sida/page/p…) i list-HTML — viktigt när konstruerad ?sida=N inte SSR:as. */
...
/** Följer Bolagsfaktas paginering (rel="next" / ”Nästa”) när query-parametrar inte räcker. */
...
async function upsertForetagWithCustomer(pipelineId: string, f: Foretag) {
  ...
  const isRedlisted = Boolean(redlistHit) || orgFormatRedlist

  let customer =
    orgVars.length > 0
      ? await prisma.customer.findFirst({ where: { orgNumber: { in: orgVars } } })
      : null
  if (!customer && !isRedlisted) {
...
/** Grovt antal förekomster av org.nr-path i HTML (för logg — kan skilja från parser). */
