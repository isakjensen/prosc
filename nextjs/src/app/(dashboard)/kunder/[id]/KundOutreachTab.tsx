...
export default function KundOutreachTab({ customerId }: { customerId: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/kunder/${customerId}/outreach`)
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/kunder/${customerId}/outreach/${editItem.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function handleDelete(id: string) {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/kunder/${customerId}/outreach/${id}`, {
        method: "DELETE",
      })
  ...
  async function toggleStatus(item: OutreachItem) {
    ...
    try {
      const res = await fetch(
        `/api/kunder/${customerId}/outreach/${item.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
