...
export default function KundProjektTab({
  ...
  async function handleLink() {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/projekt/${selectedId}/kunder`, {
        method: "POST",
  ...
  async function handleUnlink(projectId: string) {
    try {
      const res = await fetch(`/api/projekt/${projectId}/kunder/${customerId}`, {
        method: "DELETE",
      })
      if (!res.ok) throw new Error()
    ...
    <div className="space-y-4">
      ...
      {allProjects.length === 0 && (
        <p className="text-sm text-gray-500">
          ...
          <Link href="/projekt/ny" className="font-medium text-zinc-800 hover:underline">
            Skapa projekt
          </Link>
        </p>
      )}
