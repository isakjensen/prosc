...
export default function KundFlodeTab({ customerId }: { customerId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/customers/${customerId}/flow`)
      if (!res.ok) {
  ...
  async function submitNote(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/customers/${customerId}/flow/notes`, {
        method: "POST",
    ...
    try {
      if (dateModal.kind === "custom_note") {
        const res = await fetch(
          `/api/customers/${customerId}/flow/notes/${dateModal.sourceId}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
        ...
        const res = await fetch(`/api/customers/${customerId}/flow/overrides`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            kind: dateModal.kind,
  ...
  async function saveNoteEdit() {
    ...
    try {
      const res = await fetch(
        `/api/customers/${customerId}/flow/notes/${noteModal.sourceId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
