...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			...
			<span class="mt-2 inline-flex rounded-full bg-green-100 px-3 py-1 text-sm font-medium text-green-800">Kund</span>
		</div>
		<div class="flex gap-2">
			<a href="/dashboard/customers/{data.company.id}/systembehov">
				<Button variant="outline">Systembehov</Button>
			</a>
	...
	<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
						...
						<div>
							...
							<dd class="mt-1 text-sm text-gray-900">
								{#if data.company.website}
									<a href={data.company.website} target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-700">
		...
		<div class="space-y-6">
			...
			{#if data.company.contacts.length > 0}
				<Card class="p-6">
					<h2 class="text-lg font-semibold text-gray-900 mb-4">Kontakter</h2>
					<ul class="space-y-2">
			...
			{#if data.company.quotes.length > 0}
				<Card class="p-6">
					<h2 class="text-lg font-semibold text-gray-900 mb-4">Offerter</h2>
					<ul class="space-y-2">
						{#each data.company.quotes as quote}
							<li>
								<a href="/dashboard/quotes/{quote.id}" class="text-blue-600 hover:underline">
									{quote.number} – {quote.title}
								</a>
								<span class="text-sm text-gray-500">({quote.total.toFixed(0)} kr)</span>
							</li>
						{/each}
					</ul>
				</Card>
